...
Key Vault に Secret を置いても、ロー��ーションできなければ古いパスワードが長く残ります。特にデータベース接続文字列や外部 API キーは、更新時にアプリが止まる設計だと誰も回したがりません。
...
ここを甘く見ると、アプリは動いているのに Secret 取得だけ失敗する、という地味に面倒な障害になります。閉域化は強い選択ですが、運用コストも一緒に持つ判断です。
...
- ローテーション時にアプリがどう再読み込みするか���まっているか
