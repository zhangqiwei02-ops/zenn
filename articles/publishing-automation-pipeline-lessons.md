---
title: "NotionからQiita/Zennへ記事を自動公開するパイプラ��ンを作り直して学んだこと"
emoji: "🚢"
type: "tech"
...
なので、Notionには「状態」と「公開URL」を置き、ZennはGitHub repoを実体として扱う、という分担にしました。
...
対策として、DB IDを設定ファイルに寄せ、実行ログにも必ずDB IDを出すよう���しました。
...
これは地味ですが、事故率をかなり下げます。自動化は速いので、失敗も速いで���。まとめて壊れる設計にしない方がいいです。
...
QAチェックは、点数を付けるためではなく、投稿前に止めるためにあります。
...
本当に難しいのは、状態管理、成功判定、失敗時の切り分け、プラットフォーム差分の扱いです。
